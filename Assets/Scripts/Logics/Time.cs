...
namespace TRIdle.Logics
{
  public static class Time
  {
    ...
    private static readonly Dictionary<object, float> s_Cache = new();
    ...
    public static bool TickElapsed(object obj) {
      ...
      if (s_Cache.TryGetValue(obj, out var time)) {
        if (UTime.realtimeSinceStartup - time < MaxTickDuration)
          return true;
        s_Cache[obj] = UTime.realtimeSinceStartup;
        return false;
      }
      else {
        s_Cache.Add(obj, UTime.realtimeSinceStartup);
        return false;
      }
    }
  }
